...
import time
BASE = Path(os.getcwd())
TOP20_DIR = BASE / "Top 10 Quality stock"
DAYWISE_DIR = BASE / "daywise stocks"
OUTPUT_FILE = BASE / "NIFTY_LargeMidcap_250_quality_tracker.xlsx"
...
top20_files = sorted(TOP20_DIR.glob("top10_quality_*.csv"))
rebalance_dates = []
rebalance_data = {}
...
for index, rebalance_date in enumerate(rebalance_dates):
    ...
    quality_lookup = dict(zip(top20_df["Symbol"].str.strip(), top20_df["QualityScore"]))
    ...
    for symbol in sorted(holds):
        ...
        if price and symbol in portfolio:
            ...
            log_trade(rebalance_date, symbol, "HOLD", portfolio[symbol]["qty"], price,
                      0, cash, cash, f"Q: {quality_lookup.get(symbol, 0):.4f}, PnL%: {pnl_pct:.2%}")
    ...
    if buy_list:
        ...
        for symbol in buy_list:
            ...
            score = quality_lookup.get(symbol, 0)
            ...
            log_trade(rebalance_date, symbol, "BUY", qty, price,
                        brokerage, cash_before, cash, f"Rebalance entry Q:{score:.4f}")
...
if not daily_df.empty:
    ...
    labels_vals = [
        ("Strategy", "Quality Top 20, No Stop-Loss"),
