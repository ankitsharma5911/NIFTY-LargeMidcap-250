...
import time
BASE = Path(os.getcwd())
TOP20_DIR = BASE / "Top 20 positive momentum 60 days low vol 40 days stock"
DAYWISE_DIR = BASE / "daywise stocks"
OUTPUT_FILE = BASE / "NIFTY_LargeMidcap_250_positive_momentum_60_low_vol_40_tracker.xlsx"
...
top20_files = sorted(TOP20_DIR.glob("top20_mom_lowvol_*.csv"))
rebalance_dates = []
rebalance_data = {}
...
for index, rebalance_date in enumerate(rebalance_dates):
    ...
    vol_lookup = dict(zip(top20_df["Symbol"].str.strip(), top20_df["Volatility"]))
    ...
    for symbol in sorted(holds):
        ...
        if price and symbol in portfolio:
            ...
            log_trade(rebalance_date, symbol, "HOLD", portfolio[symbol]["qty"], price, 
                      0, cash, cash, f"Vol: {vol_lookup.get(symbol, 0):.4f}, PnL%: {pnl_pct:.2%}")
    ...
    if buy_list:
        ...
        for symbol in buy_list:
            ...
            log_trade(rebalance_date, symbol, "BUY", qty, price, 
                        brokerage, cash_before, cash, f"Rebalance entry Vol:{vol:.4f}")
...
if not daily_df.empty:
    ...
    labels_vals = [
        ("Strategy", "Top 20 Momentum Low Vol, No Stop-Loss"),
