...
import time
BASE = Path(os.getcwd())
TOP20_DIR = BASE / "Top 20 Alpha stock"
DAYWISE_DIR = BASE / "daywise stocks"
OUTPUT_FILE = BASE / "NIFTY_LargeMidcap_250_alpha_tracker.xlsx"
...
top20_files = sorted(TOP20_DIR.glob("top20_alpha_*.csv"))
rebalance_dates = []
rebalance_data = {}
...
for index, rebalance_date in enumerate(rebalance_dates):
    ...
    alpha_lookup = dict(zip(top20_df["Symbol"].str.strip(), top20_df["Alpha"]))
    ...
    for symbol in sorted(holds):
        ...
        if price and symbol in portfolio:
            ...
            log_trade(rebalance_date, symbol, "HOLD", portfolio[symbol]["qty"], price,
                      0, cash, cash, f"Alpha: {alpha_lookup.get(symbol, 0):.4f}, PnL%: {pnl_pct:.2%}")
    ...
    if buy_list:
        ...
        for symbol in buy_list:
            ...
            alpha = alpha_lookup.get(symbol, 0)
            ...
            log_trade(rebalance_date, symbol, "BUY", qty, price,
                        brokerage, cash_before, cash, f"Rebalance entry Alpha:{alpha:.4f}")
...
if not daily_df.empty:
    ...
    labels_vals = [
        ("Strategy", "Alpha Top 20, No Stop-Loss"),
