...
import time
BASE = Path(os.getcwd())
TOP20_DIR = BASE / "Top 20 NSE Momentum"
DAYWISE_DIR = BASE / "daywise stocks"
OUTPUT_FILE = BASE / "NIFTY_LargeMidcap_250_NSE_momentum_tracker.xlsx"
...
top20_files = sorted(TOP20_DIR.glob("top20_momentum_*.csv"))
rebalance_dates = []
rebalance_data = {}
...
for index, rebalance_date in enumerate(rebalance_dates):
    ...
    vol_lookup = dict(zip(top20_df["Symbol"].str.strip(), top20_df["MomentumScore"]))
    
    if index + 1 < len(rebalance_dates):
        next_rebalance_date = rebalance_dates[index + 1]
    else:
        next_rebalance_date = rebalance_date + pd.Timedelta(days=120)
    ...
    for symbol in sorted(holds):
        ...
        if price and symbol in portfolio:
            ...
            log_trade(rebalance_date, symbol, "HOLD", portfolio[symbol]["qty"], price, 
                      0, cash, cash, f"Momentum: {vol_lookup.get(symbol, 0):.4f}, PnL%: {pnl_pct:.2%}")
...
if not daily_df.empty:
    ...
    labels_vals = [
        ("Strategy", "Momentum Top 20, No Stop-Loss"),
