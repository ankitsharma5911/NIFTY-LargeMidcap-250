...
OUTPUT_DIR = BASE / "Top 20 momentum 70 days low vol 70 days stock"

OUTPUT_DIR.mkdir(exist_ok=True)

LOOKBACK = 70      # momentum lookback (trading days)
ROLLING_WINDOW = 70  # volatility rolling window
...
for rb_file in rebalance_files:
    ...
    if rows:
        ...
        print(f"  Saved: {out_file.name} | Top stock: {result_df.iloc[0]['Symbol']} (Score: {result_df.iloc[0]['Score']})")
    else:
